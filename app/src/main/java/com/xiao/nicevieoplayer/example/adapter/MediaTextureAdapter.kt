...
import com.xiao.nicevieoplayer.example.adapter.holder.MediaTextureHolder
import com.xiao.nicevieoplayer.example.bean.Video

class MediaTextureAdapter(private val mContext: Context, private val mVideoList: List<Video>) :
    RecyclerView.Adapter<MediaTextureHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MediaTextureHolder {
        val itemView =
            LayoutInflater.from(mContext).inflate(R.layout.item_media_texture_video, parent, false)
        val holder = MediaTextureHolder(itemView)
    ...
    override fun onBindViewHolder(holder: MediaTextureHolder, position: Int) {
