...
public class AliTextureListActivity extends CompatHomeKeyActivity
        implements ScreenRotateUtils.OrientationChangeListener {
    ...
    private void init() {
        ...
        AliAdapter adapter = new AliAdapter(this, DataUtil.getVideoListData(), true);
    ...
    protected void onDestroy() {
        super.onDestroy();
        ScreenRotateUtils.getInstance(this.getApplicationContext())
    ...
    public void orientationChange(int orientation) {
        if (NiceVideoPlayerManager.instance().getCurrentNiceVideoPlayer() != null
                && (NiceVideoPlayerManager.instance().getCurrentNiceVideoPlayer().isPaused()
            ...
            if (orientation >= 45 && orientation <= 315 && NiceVideoPlayerManager.instance()
                                                                                 .getCurrentNiceVideoPlayer()
                                                                                 .isNormal()) {
                NiceVideoPlayerManager.instance().getCurrentNiceVideoPlayer().enterFullScreen();
                    ...
                    && NiceVideoPlayerManager.instance().getCurrentNiceVideoPlayer()
                                             .isFullScreen()) {
